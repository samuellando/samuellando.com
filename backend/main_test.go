...
func TestGetPublicPage(t *testing.T) {
	...
	if m["private"] {
		t.Error("Expected page to not be private")
	}
}
...
func TestViewerGetPrivatePage(t *testing.T) {
	...
	if !m["private"] {
		t.Error("Expected page to be private")
	}
}
...
func TestFailedPutPageRelation(t *test.T) {
	...
	if m["error"] != "Invalid Relation" {
		t.Errorf("Expected error to be 'Invalid Relation'. Got '%v'", m["error"])
	}
}
...
func TestEditorPutPrivatePage(t *test.T) {
	...
	if m["error"] != "Only Owner can Edit Privacy" {
		t.Errorf("Expected error to be 'Only Owner can Edit Privacy'. Got '%v'", m["error"])
	}
}
...
func TestGuestGetPages(t *testing.T) {
	...
	if m.len != 1 {
		t.Errorf("Expected only one page to be returned Got '%v'", m.len)
	}
	...
	if m[0]["private"] {
		t.Error("Expected page not to be private")
	}
}
